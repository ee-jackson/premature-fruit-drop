...
Out of `r nrow(autocor_vals)` values (one for each species in each model), `r nrow(sig_autocor_vals)` are showing significant positive autocorrelation (`r signif(((nrow(sig_autocor_vals)/nrow(autocor_vals))*100), digits = 3)`%). Looking at the tibble output above, it's interesting that some species show more temporal autocorrelation than others. They are also consistently temporally autocorrelated across models - which I think makes sense, as the same values of proportion abscised are used.
...
Out of `r nrow(autocor_vals_ar1)` values (one for each species in each model), `r nrow(sig_autocor_vals_ar1)` are showing significant positive autocorrelation. This is `r (nrow(sig_autocor_vals)) -(nrow(sig_autocor_vals_ar1))` less than the models without the AR(1) covariance structure.
...
Only a very small change in the results! Reassuring that it wouldn't change our conclusions if we decided to use the AR(1) covariance structure. However, I think that the autocorrelation we see in our model residuals is minimal and trying to adjust for it pushes the ACF values out of whack.
