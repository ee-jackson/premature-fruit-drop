...
propDat.sp <- propDat %>%
	group_by(sp) %>% 
	summarise(proportion_abscised = mean(proportion_abscised), abscised_seeds = sum(abscised_seeds), viable_seeds = sum(viable_seeds), total_seeds = sum(total_seeds),n_years = length(unique(year))) %>%
	ungroup()
...
plantTraits <- subset(propDat.traits, select = c(sp, proportion_abscised, abscised_seeds, viable_seeds, total_seeds, n_years FAMILY, GENUS, SPECIES, LIFEFORM, N_SEEDFULL, SEED_DRY, HEIGHT_AVG, cvseed, CoFruit, BCIReproductive, Endocarp_investment, Plant_species19, SeedPredationRate, SeedPred_n, SeedPred_pres, Coleo_pres, Hymeno_pres, Lepid_pres))
